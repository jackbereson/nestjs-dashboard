...
const ModalUpdateCustomerBalance = ({
  data,
  open,
  setOpen,
  loadData,
  ...
  const submit = (
    ...
    { setSubmitting }: FormikHelpers<CustomerBalance>
  ) => {
    updateData(values);
  ...
  const updateData = (values: CustomerBalance) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateCustomerBalanceInput = {
      coin: values.coin,
    };

    CustomerBalanceService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: CustomerBalance) => {
...
export default ModalUpdateCustomerBalance;
