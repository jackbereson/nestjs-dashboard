...
const ModalCreateStakingProduct = ({
  open,
  setOpen,
  loadData,
}: {
  ...
  const submit = (
    values: CreateStakingProductInput,
    { setSubmitting }: FormikHelpers<CreateStakingProductInput>
  ) => {
    createData({...values});
  ...
  const createData = (data: CreateStakingProductInput) => {
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    StakingProductService.create({ data, token })
      .then(() => {
        loadData();
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Create new stakingProduct"
    >
      <Formik
        initialValues={StakingProductInitialValues}
...
export default ModalCreateStakingProduct;
