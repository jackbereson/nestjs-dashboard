...
const ModalCreateSportBet = ({
  ...
  const submit = (
    values: CreateSportBetInput,
    { setSubmitting }: FormikHelpers<CreateSportBetInput>
  ) => {
    createData({...values});
  ...
  const createData = (data: CreateSportBetInput) => {
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    SportBetService.create({ data, token })
      .then(() => {
        loadData();
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Create new sportBet"
    >
      <Formik
        initialValues={SportBetInitialValues}
      ...
      >
        {({ errors, touched, values }) => (
          <Form>
            <div className="p-3">
              <InputField
                labelName={SportBetArgNames.name}
                inputName={SportBetArgs.name}
                placeholder={`Input ${SportBetArgNames.name}`}
...
export default ModalCreateSportBet;
