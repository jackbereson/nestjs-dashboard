...
import {
  ...
  NftLauchpadPackageInitialValues,
  CreateNftLauchpadPackageInput,
...
const ModalCreateNftLauchpadPackage = ({
  open,
  setOpen,
  loadData,
}: {
  ...
  const submit = (
    values: CreateNftLauchpadPackageInput,
    { setSubmitting }: FormikHelpers<CreateNftLauchpadPackageInput>
  ) => {
    createData({...values});
  ...
  const createData = (data: CreateNftLauchpadPackageInput) => {
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    NftLauchpadPackageService.create({ data, token })
      .then(() => {
        loadData();
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Create new nftLauchpadPackage"
    >
      <Formik
        initialValues={NftLauchpadPackageInitialValues}
...
export default ModalCreateNftLauchpadPackage;
