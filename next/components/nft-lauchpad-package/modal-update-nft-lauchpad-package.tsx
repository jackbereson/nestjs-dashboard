...
const ModalUpdateNftLauchpadPackage = ({
  data,
  open,
  setOpen,
  loadData,
  ...
  const submit = (
    values: NftLauchpadPackage,
    { setSubmitting }: FormikHelpers<NftLauchpadPackage>
  ) => {
    updateData(values);
  ...
  const updateData = (values: NftLauchpadPackage) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateNftLauchpadPackageInput = {
      name: values.name,
    };

    NftLauchpadPackageService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: NftLauchpadPackage) => {
    const data: NftLauchpadPackage = {
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Update nftLauchpadPackage"
    >
      <Formik
      ...
      >
        {({ errors, touched, values, setFieldValue }) => (
          <Form>
            <div className="p-3">
              <InputField
                labelName={NftLauchpadPackageArgNames.name}
                inputName={NftLauchpadPackageArgs.name}
                placeholder={`Input ${NftLauchpadPackageArgNames.name}`}
...
export default ModalUpdateNftLauchpadPackage;
