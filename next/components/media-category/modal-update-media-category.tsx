...
const ModalUpdateMediaCategory = ({
  data,
  open,
  setOpen,
  loadData,
  ...
  const submit = (
    ...
    { setSubmitting }: FormikHelpers<MediaCategory>
  ) => {
    updateData(values);
  ...
  const updateData = (values: MediaCategory) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateMediaCategoryInput = {
      name: values.name,
    };

    MediaCategoryService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: MediaCategory) => {
    const data: MediaCategory = {
...
export default ModalUpdateMediaCategory;
