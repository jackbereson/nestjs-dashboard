...
const ModalCreateMediaCategory = ({
  open,
  setOpen,
  loadData,
}: {
  ...
  const submit = (
    values: CreateMediaCategoryInput,
    { setSubmitting }: FormikHelpers<CreateMediaCategoryInput>
  ) => {
    createData({...values});
  ...
  const createData = (data: CreateMediaCategoryInput) => {
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    MediaCategoryService.create({ data, token })
      .then(() => {
        loadData();
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Create new mediaCategory"
    >
      <Formik
        initialValues={MediaCategoryInitialValues}
      ...
      >
        {({ errors, touched, values }) => (
          <Form>
            <div className="p-3">
              <InputField
                labelName={MediaCategoryArgNames.name}
                inputName={MediaCategoryArgs.name}
                placeholder={`Input ${MediaCategoryArgNames.name}`}
...
export default ModalCreateMediaCategory;
