...
const ModalUpdateNftLaunchpad = ({
  ...
  const submit = (
    values: NftLaunchpad,
    { setSubmitting }: FormikHelpers<NftLaunchpad>
  ) => {
    updateData(values);
  ...
  const updateData = (values: NftLaunchpad) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateNftLaunchpadInput = {
      name: values.name,
    };

    NftLaunchpadService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: NftLaunchpad) => {
    const data: NftLaunchpad = {
...
export default ModalUpdateNftLaunchpad;
