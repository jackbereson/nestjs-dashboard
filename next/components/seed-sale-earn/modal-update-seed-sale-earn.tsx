...
const ModalUpdateSeedSaleEarn = ({
  ...
  const submit = (
    values: SeedSaleEarn,
    { setSubmitting }: FormikHelpers<SeedSaleEarn>
  ) => {
    updateData(values);
  ...
  const updateData = (values: SeedSaleEarn) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateSeedSaleEarnInput = {
      name: values.name,
    };

    SeedSaleEarnService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: SeedSaleEarn) => {
    const data: SeedSaleEarn = {
...
export default ModalUpdateSeedSaleEarn;
