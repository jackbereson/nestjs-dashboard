...
const ModalUpdateStudent = ({
  ...
  const submit = (
    values: Student,
    { setSubmitting }: FormikHelpers<Student>
  ) => {
    updateData(values);
  ...
  const updateData = (values: Student) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateStudentInput = {
      name: values.name,
    };

    StudentService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: Student) => {
    const data: Student = {
