...
const ModalUpdateSport = ({
  ...
  const submit = (
    values: Sport,
    { setSubmitting }: FormikHelpers<Sport>
  ) => {
    updateData(values);
  ...
  const updateData = (values: Sport) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateSportInput = {
      name: values.name,
    };

    SportService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: Sport) => {
    const data: Sport = {
      ...
      >
        {({ errors, touched, values, setFieldValue }) => (
          <Form>
            <div className="p-3">
              <InputField
                labelName={SportArgNames.name}
                inputName={SportArgs.name}
                placeholder={`Input ${SportArgNames.name}`}
...
export default ModalUpdateSport;
