...
const ModalCreateNftCollection = ({
  open,
  setOpen,
  loadData,
}: {
  ...
  const submit = (
    values: CreateNftCollectionInput,
    { setSubmitting }: FormikHelpers<CreateNftCollectionInput>
  ) => {
    createData({...values});
  ...
  const createData = (data: CreateNftCollectionInput) => {
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    NftCollectionService.create({ data, token })
      .then(() => {
        loadData();
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Create new nftCollection"
    >
      <Formik
        initialValues={NftCollectionInitialValues}
...
export default ModalCreateNftCollection;
