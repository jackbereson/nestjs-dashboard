...
import { BalanceTransactionService } from "../../lib/modules/balance-transaction/balance-transaction.repo";
...
const ModalUpdateBalanceTransaction = ({
  data,
  open,
  setOpen,
  loadData,
  ...
  const submit = (
    values: BalanceTransaction,
    { setSubmitting }: FormikHelpers<BalanceTransaction>
  ) => {
    updateData(values);
  ...
  const updateData = (values: BalanceTransaction) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateBalanceTransactionInput = {
      coin: values.coin,
    };

    BalanceTransactionService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: BalanceTransaction) => {
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Update balanceTransaction"
    >
      <Formik
      ...
      >
        {({ errors, touched, values, setFieldValue }) => (
          <Form>
            <div className="p-3">
              <InputField
                labelName={BalanceTransactionArgNames.coin}
                inputName={BalanceTransactionArgs.coin}
                placeholder={`Input ${BalanceTransactionArgNames.coin}`}
...
export default ModalUpdateBalanceTransaction;
