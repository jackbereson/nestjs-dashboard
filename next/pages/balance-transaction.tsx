...
import BalanceTransactionTableList from "../components/balance-transaction/balance-transaction-table-list";

const BalanceTransactionPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);

  const [page, setPage] = useState(1);
  const [data, setData] = useState<BalanceTransaction[]>(null);
  ...
  const getData = async ({ page, length }: { page: number; length: number }) => {
    ...
    BalanceTransactionService.getAll({
      query: {
        limit: length,
      ...
      token,
      cache: false,
      fragment: BalanceTransactionService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
BalanceTransactionPage.Layout = AdminLayout;
...
const Table = ({
  ...
  return (
    <div className="bg-seconds-light pb-1">
      <TableListWrapper>
        <BalanceTransactionTableList data={data} loadData={loadData} viewMode={viewMode} />
      </TableListWrapper>
      <Paging
