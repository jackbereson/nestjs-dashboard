...
import NftInventoryTableList from "../components/nft-inventory/nft-inventory-table-list";
import NftInventoryActionCreate from "../components/nft-inventory/nft-inventory-action-create";

const NftInventoryPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<NftInventory[]>(null);
  ...
  const getData = async ({
    ...
    NftInventoryService.getAll({
      ...
      token,
      cache: false,
      fragment: NftInventoryService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
NftInventoryPage.Layout = AdminLayout;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <NftInventoryActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
