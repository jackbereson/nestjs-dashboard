...
import StakingProductTableList from "../components/staking-product/staking-product-table-list";
import StakingProductActionCreate from "../components/staking-product/staking-product-action-create";

const StakingProductPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<StakingProduct[]>(null);
  ...
  const getData = async ({
    ...
    StakingProductService.getAll({
      ...
      token,
      cache: false,
      fragment: StakingProductService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
StakingProductPage.Layout = AdminLayout;
export default StakingProductPage;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <StakingProductActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
