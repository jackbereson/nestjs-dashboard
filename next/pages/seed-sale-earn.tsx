...
import SeedSaleEarnTableList from "../components/seed-sale-earn/seed-sale-earn-table-list";
import SeedSaleEarnActionCreate from "../components/seed-sale-earn/seed-sale-earn-action-create";

const SeedSaleEarnPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<SeedSaleEarn[]>(null);
  ...
  const getData = async ({
    ...
    SeedSaleEarnService.getAll({
      ...
      token,
      cache: false,
      fragment: SeedSaleEarnService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
SeedSaleEarnPage.Layout = AdminLayout;
export default SeedSaleEarnPage;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <SeedSaleEarnActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
