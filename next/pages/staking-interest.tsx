...
import StakingInterestTableList from "../components/staking-interest/staking-interest-table-list";
import StakingInterestActionCreate from "../components/staking-interest/staking-interest-action-create";

const StakingInterestPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<StakingInterest[]>(null);
  ...
  const getData = async ({
    ...
    StakingInterestService.getAll({
      ...
      token,
      cache: false,
      fragment: StakingInterestService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
StakingInterestPage.Layout = AdminLayout;
export default StakingInterestPage;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <StakingInterestActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
