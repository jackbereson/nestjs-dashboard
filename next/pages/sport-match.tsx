...
import SportMatchTableList from "../components/sport-match/sport-match-table-list";
import SportMatchActionCreate from "../components/sport-match/sport-match-action-create";

const SportMatchPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<SportMatch[]>(null);
  ...
  const getData = async ({
    ...
    SportMatchService.getAll({
      ...
      token,
      cache: false,
      fragment: SportMatchService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
SportMatchPage.Layout = AdminLayout;
export default SportMatchPage;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <SportMatchActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
