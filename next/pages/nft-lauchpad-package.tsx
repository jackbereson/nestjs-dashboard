...
import NftLauchpadPackageTableList from "../components/nft-lauchpad-package/nft-lauchpad-package-table-list";
import NftLauchpadPackageActionCreate from "../components/nft-lauchpad-package/nft-lauchpad-package-action-create";

const NftLauchpadPackagePage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<NftLauchpadPackage[]>(null);
  ...
  const getData = async ({
    ...
    NftLauchpadPackageService.getAll({
      query: {
        limit: length,
      ...
      token,
      cache: false,
      fragment: NftLauchpadPackageService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
NftLauchpadPackagePage.Layout = AdminLayout;
export default NftLauchpadPackagePage;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <NftLauchpadPackageActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
