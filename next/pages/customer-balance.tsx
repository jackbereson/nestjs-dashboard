...
import CustomerBalanceTableList from "../components/customer-balance/customer-balance-table-list";
import CustomerBalanceActionCreate from "../components/customer-balance/customer-balance-action-create";

const CustomerBalancePage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<CustomerBalance[]>(null);
  ...
  const getData = async ({
    ...
    CustomerBalanceService.getAll({
      ...
      token,
      cache: false,
      fragment: CustomerBalanceService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
CustomerBalancePage.Layout = AdminLayout;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <CustomerBalanceActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
