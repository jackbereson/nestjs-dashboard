...
import NftLaunchpadTableList from "../components/nft-launchpad/nft-launchpad-table-list";
import NftLaunchpadActionCreate from "../components/nft-launchpad/nft-launchpad-action-create";

const NftLaunchpadPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<NftLaunchpad[]>(null);
  ...
  const getData = async ({
    ...
    NftLaunchpadService.getAll({
      ...
      token,
      cache: false,
      fragment: NftLaunchpadService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
NftLaunchpadPage.Layout = AdminLayout;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <NftLaunchpadActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
